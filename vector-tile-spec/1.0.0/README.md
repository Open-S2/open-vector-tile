...
```ts
const CommandInteger = (id & 0x7) | (count << 3)
```
...
| MoveTo           | `1`  | `dX`, `dY`                                 | 2               |
| LineTo           | `2`  | `dX`, `dY`                                 | 2               |
| bezierCurveTo    | `5`  | `dX`, `dY`, `C1dX`, `C1dY`, `C2dX`, `C2dY` | 6               |
| quadraticCurveTo | `6`  | `dX`, `dY`, `CdX`, `CdY`                   | 4               |
| ClosePath        | `7`  | No parameters                              | 0               |
| ClosePolygon     | `4`  | No parameters                              | 0               |
...
| ClosePolygon | `4`  | `1`   | `12`           | `[00000000 00000000 00000000 00001][100]` |
...
##### 4.3.3.4. ClosePolygon Command

a `ClosePolygon` command MUST have a command count of 1 and no parameters. the command ends the current polygon of a MULTIPOLYGON geometry. The following polygon will have a `MoveTo` command that starts at the last point described by the previous polygon.
...
3. A `ClosePath` with no parameters
4. A `ClosePolygon` with no parameters
...
Following a full set of `ExteriorRing` and potential `InteriorRing`s, a `ClosePolygon` command will be given. This is to properly separate each subsequent polygon.
...
* ClosePath
* ClosePolygon // End of Polygon 1
* MoveTo(+11,+1) // NOTE THAT THIS IS RELATIVE TO LAST LINETO!
* LineTo(+9,+0)
* LineTo(+0,+9)
* LineTo(-9,+0) // Cursor at 11,20 after this command
* ClosePath // Because the next ring is an interior one, we do not run the "ClosePolygon" command
...
For example, a GeoJSON feature could be structured like:
